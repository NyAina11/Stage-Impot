...
const ChefDivisionView: React.FC = () => {
    ...
    const [dossierToCancel, setDossierToCancel] = useState<DossierPaiement | null>(null);
    ...
    const filteredDossiers = useMemo(() => {
        return dossiers.filter(d => {
            const searchTermMatch = filters.searchTerm === '' ||
                ...
                d.taxId.toLowerCase().includes(filters.searchTerm.toLowerCase());

            const statusMatch = filters.status === '' || d.status === filters.status;

            const date = new Date(d.creationDate);
                        ...
                        <tbody>
                            {filteredDossiers.map(d => (
                                <tr key={d.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className="px-6 py-4">{d.amountDue ? `${d.amountDue.toLocaleString('fr-FR')} Ar` : 'N/A'}</td>
                                    <td className="px-6 py-4"><Badge status={d.status} /></td>
                                    <td className="px-6 py-4">{new Date(d.creationDate).toLocaleDateString('fr-FR')}</td>
                                    <td className="px-6 py-4 flex space-x-2">
                                        <Button variant="secondary" onClick={() => setSelectedDossier(d)}>Détails</Button>
